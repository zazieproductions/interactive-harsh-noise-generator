...
Before opening a PR, run the local build checks:

- `npm install`
- `npm run build`
- Optional test/lint steps as those tools land

If a hosted CI workflow is added later and fails on your PR, pull the latest `main`, rebase or
merge, and re-run locally with `npm run build` to reproduce the failure.
