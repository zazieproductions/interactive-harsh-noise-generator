...
**NOISE WALL** is a browser-based creative-coding tool by **Zazie Productions LLC** that
generates reproducible harsh-noise-wall (HNW) textures using a custom DSP pipeline built on
top of the Web Audio API and the `<canvas>` element.
...
| **Single-file deploy** | One HTML file that is easy to host, share, or open with `file://` |
| **Focused UI** | Dark, high-contrast interface with clear controls and an animated waveform |
...
- Run `npm run build` before opening a PR to catch type/build regressions.
